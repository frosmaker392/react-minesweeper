...
describe('Board', () => {
  ...
  describe('reveal', () => {
    describe('init', () => {
      const checkRevealed = (board: BoardLogic, coords: [number, number]): void => {
      ...
      it('randomly installs mines to all except the cell and its neighbors, then reveals it', () => {
        ...
        const coordsToEval: Array<[number, number]> = [
          [1, 1], [2, 1], [3, 1],
          [1, 2], [2, 2], [3, 2],
          [1, 3], [2, 3], [3, 3]
        ]
  ...
  describe('state', () => {
    ...
    beforeEach(() => {
      ...
      board.initialized = true
      for (let x = 0; x < 5; x++) { board.at([x, 2]).hasMine = true }
    })
    ...
    it('is "won" when all mines are flagged and other cells are revealed', () => {
      ...
      for (let x = 0; x < 5; x++) { board = board.mark([x, 2]) }
