...
type MarkState = Exclude<CellState, 'revealed'>
...
class BoardLogic implements IBoard {
  ...
  private readonly nextState: Record<MarkState, MarkState> = {
    hidden: 'flagged',
    flagged: 'unknown',
    unknown: 'hidden',
  }
  ...
  public mark(coords: Coords): BoardLogic {
    ...
    cell.state = this.nextState[cell.state as MarkState]
  ...
  public reveal(coords: Coords): BoardLogic {
    ...
    while (queue.length > 0) {
      ...
      const nMines = neighbors.filter(
        (nCoords) => newBoard.at(nCoords).hasMine
      ).length
      const nFlags = neighbors.filter(
        (nCoords) => newBoard.at(nCoords).state === 'flagged'
      ).length
  ...
  public state(): BoardState {
    ...
    for (const row of this.cells) {
      for (const cell of row) {
        hasWon =
          hasWon &&
          (cell.hasMine ? cell.state === 'flagged' : cell.state === 'revealed')
  ...
  public flagCount(): number {
    return this.cells.reduce(
      (cnt, row) =>
        cnt +
        row.reduce((rowCnt, cell) => rowCnt + +(cell.state === 'flagged'), 0),
      0
    )
  }
  ...
  public debug(): void {
    console.log(
      this.cells
        .map((row) =>
          row
            .map((cell) => `${cell.state === 'revealed' ? 'X' : 'O'}`)
            .join(' ')
        )
        .join('\n')
    )
